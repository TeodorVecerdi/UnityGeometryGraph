...
namespace GeometryGraph.Editor {
    ...
    public class FloatMathNode : AbstractNode<GeometryGraph.Runtime.Graph.FloatMathNode> {
        ...
        private void OnOperationChanged() {
            ...
            if (showExtra) {
                extraPort.RemoveFromClassList("d-none");
            } else {
                extraPort.AddToClassList("d-none");
                extraPort.connections.Select(edge => (SerializedEdge)edge.userData).ToList().ForEach(edge => Owner.EditorView.GraphView.GraphData.RemoveEdge(edge));
                extraPort.DisconnectAll();
            }

            if (showY) {
                yPort.RemoveFromClassList("d-none");
            } else {
                yPort.AddToClassList("d-none");
                yPort.connections.Select(edge => (SerializedEdge)edge.userData).ToList().ForEach(edge => Owner.EditorView.GraphView.GraphData.RemoveEdge(edge));
                yPort.DisconnectAll();
            }
